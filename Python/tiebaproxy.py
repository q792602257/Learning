...
while True:
	conn,addr=s.accept()
	recv=conn.recv(131072).replace("127.0.0.1","tieba.baidu.com")
	print recv
	if len(recv)!=0:
		ret=dataHandle(recv.split("\r\n")).replace("https://passport.baidu.com","http://127.0.0.1:880").replace("http://passport.baidu.com","http://127.0.0.1:880")
		conn.send(ret)
	conn.close()
